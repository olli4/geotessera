...
class Registry:
    ...
    def load_all_blocks(self, progress_callback: Optional[callable] = None):
        ...
        try:
            if self._registry_dir:
                ...
                for i, block_file in enumerate(block_files):
                    if progress_callback:
                        progress_callback(i, total_blocks, f"Loading registry blocks: {i + 1}/{total_blocks}")
            ...
            if progress_callback:
                progress_callback(total_blocks, total_blocks, f"Loaded {len(self._available_embeddings)} total embeddings")
            else:
    ...
    def fetch(self, path: str, progressbar: bool = True, progress_callback: Optional[callable] = None) -> str:
    ...
    def fetch_landmask(self, filename: str, progressbar: bool = True, progress_callback: Optional[callable] = None) -> str:
