...
class RichHTTPDownloader:
    ...
    def __init__(self, progress_callback: Optional[Callable] = None, description: str = "Downloading"):
    ...
    def __call__(self, url: str, output_file: str, known_hash: Optional[str] = None):
        ...
        with requests.get(url, stream=True, timeout=30) as response:
            ...
            if self.progress_callback and total_size > 0:
                tile_name = extract_tile_coordinates_from_url(url)
                self.progress_callback(total_size, total_size, f"Downloaded {tile_name}")
        
        return output_file


def create_rich_downloader(progress_callback: Optional[Callable] = None, 
    ...
    """
    return RichHTTPDownloader(progress_callback=progress_callback, description=description)
