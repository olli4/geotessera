...
class CountryLookup:
    ...
    def __init__(self, cache_dir: Optional[Path] = None, progress_callback: Optional[Callable] = None):
    ...
    def _get_countries_data_path(self) -> Path:
        ...
        geojson_path = extract_dir / "natural-earth-vector-5.1.2" / "geojson" / "ne_110m_admin_0_countries.geojson"
                                ...
                                if int(mb_downloaded) > last_mb:
                                    ...
                                    if self.outer_progress_callback:
                                        if total_size > 0:
                                            ...
                                            self.outer_progress_callback(progress, 100, f"Downloading country data... {mb_downloaded:.1f}/{mb_total:.1f} MB")
                                        else:
                                            # No total size, estimate progress based on typical size (~18MB)
                                            estimated_total = 18 * 1024 * 1024  # 18MB typical size
                                            progress = min(45, int((downloaded / estimated_total) * 45))  # Cap at 45%
                                            self.outer_progress_callback(progress, 100, f"Downloading country data... {mb_downloaded:.1f} MB")
    ...
    def _resolve_country_name(self, country_name: str) -> str:
        ...
        raise ValueError(f"Country '{country_name}' not found. Use list_countries() to see available options.")
    ...
    def search_countries(self, query: str) -> List[str]:
        ...
        matches = difflib.get_close_matches(
            query.lower(),
            [name.lower() for name in all_countries],
            n=10,
            cutoff=0.3
        )
...
def get_country_bbox(country_name: str, progress_callback: Optional[Callable] = None) -> Tuple[float, float, float, float]:
